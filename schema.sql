...
-- ============================================================
-- FASE 6.1: CONFIGURAÇÃO DE SEGURANÇA (Super Admin canônico)
-- ============================================================
...
BEGIN
    IF OLD.user_id IS DISTINCT FROM NEW.user_id THEN
        RAISE EXCEPTION 'Campo user_id Ã© imutÃ¡vel em UPDATE.';
    END IF;
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        IF OLD.id IS DISTINCT FROM NEW.id THEN
            RAISE EXCEPTION 'O id do profile Ã© imutÃ¡vel.';
        END IF;

        IF OLD.is_super_admin IS DISTINCT FROM NEW.is_super_admin THEN
            RAISE EXCEPTION 'O marcador is_super_admin Ã© imutÃ¡vel.';
        END IF;
        ...
        IF v_super_admin_id IS NOT NULL AND NEW.id = v_super_admin_id THEN
            ...
            IF v_actor_id IS NOT NULL AND v_actor_id <> v_super_admin_id THEN
                RAISE EXCEPTION 'Apenas o prÃ³prio super admin pode alterar este profile.';
            END IF;
        ...
        IF OLD.role IS DISTINCT FROM NEW.role THEN
            IF NEW.role IS NULL OR NEW.role NOT IN ('admin', 'professor', 'aluno') THEN
                RAISE EXCEPTION 'Role invÃ¡lida.';
            END IF;
            ...
            IF v_actor_id = OLD.id THEN
                RAISE EXCEPTION 'UsuÃ¡rios nÃ£o podem alterar a prÃ³pria role.';
            END IF;
            ...
            RAISE EXCEPTION 'Sem permissÃ£o para alterar roles.';
        END IF;
...
BEGIN
    IF OLD.id = public.get_super_admin_user_id() OR COALESCE(OLD.is_super_admin, FALSE) = TRUE THEN
        RAISE EXCEPTION 'Super admin nÃ£o pode ser removido.';
    END IF;
...
-- FASE 1 substitui a proteção legada por UUID canônico + trigger de segurança consolidado.
DROP TRIGGER IF EXISTS block_delete_super_admin ON user_profiles;
DROP TRIGGER IF EXISTS block_update_super_admin ON user_profiles;
DROP TRIGGER IF EXISTS enforce_super_admin_role_trigger ON user_profiles;
